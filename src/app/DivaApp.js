...
/* ─── default state ─── */
function init() {
  const mkS = (items) => items.map((it) => ({ ac: Array(it.adult.length).fill(false), cc: Array(it.child.length).fill(false), ap: null, cp: null }));
  const mkD = (obj) => Object.fromEntries(Object.keys(obj).map((k) => [k, { checks: Array(obj[k].items.length).fill(false), present: null }]));
  return { inatt: mkS(INATTENTION), hyper: mkS(HYPERACTIVITY), impAdult: mkD(IMPAIRMENT.adult), impChild: mkD(IMPAIRMENT.child), onsetBefore7: null, onsetAge: "", criterionE: null, criterionEDetail: "" };
}

/* ─── tiny UI pieces ─── */
function Pill({ active, color, onClick, children }) {
  const colors = { yes: { bg:"var(--c-yes)", soft:"var(--c-yes-soft)" }, no: { bg:"var(--c-no)", soft:"var(--c-no-soft)" } };
  const c = colors[color] || colors.yes;
  return <button onClick={onClick} style={{ padding:"7px 20px", borderRadius:24, background: active ? c.bg : "var(--c-border)", color: active ? "#fff" : "var(--c-text-soft)", fontSize:13, fontWeight:600, transition:"all .15s", minWidth:60 }}>{children}</button>;
}

function Check({ on, onChange, label }) {
  return (
    <label style={{ display:"flex", alignItems:"flex-start", gap:10, padding:"8px 10px", borderRadius:8, cursor:"pointer", background: on ? "var(--c-accent-soft)" : "transparent", transition:"background .15s", fontSize:14, lineHeight:1.55, userSelect:"none" }}>
      <span style={{ display:"inline-flex", alignItems:"center", justifyContent:"center", width:22, height:22, minWidth:22, borderRadius:6, border: on ? "2px solid var(--c-check)" : "2px solid var(--c-border)", background: on ? "var(--c-check)" : "#fff", marginTop:1, transition:"all .12s", flexShrink:0 }}>
...
function SymptomCard({ item, data, onToggle, onPresent }) {
  ...
  const statusColor = both ? "var(--c-yes)" : (data.ap === true || data.cp === true) ? "var(--c-accent)" : done ? "var(--c-no)" : "var(--c-border)";

  return (
    <div style={{ background:"var(--c-card)", borderRadius:"var(--radius)", boxShadow:"var(--shadow)", marginBottom:12, overflow:"hidden", border: open ? "1.5px solid var(--c-accent)" : "1.5px solid var(--c-border)", transition:"border-color .2s" }}>
      <button onClick={() => setOpen(!open)} style={{ width:"100%", display:"flex", alignItems:"center", padding:"14px 16px", textAlign:"left", gap:12 }}>
        <span style={{ width:34, height:34, minWidth:34, borderRadius:9, background: statusColor, color:"#fff", display:"inline-flex", alignItems:"center", justifyContent:"center", fontWeight:700, fontSize:11, fontFamily:"var(--font-display)", transition:"background .2s" }}>
          {done ? "✓" : item.id}
        </span>
        <span style={{ flex:1, fontSize:14, fontWeight:500, lineHeight:1.45, color:"var(--c-text)" }}>{item.q}</span>
      ...
      {open && (
        <div style={{ padding:"0 16px 16px" }}>
          {["adult","child"].map((age) => {
            ...
            return (
              <div key={age} style={{ marginBottom: age === "adult" ? 18 : 0, ...(age === "child" ? { borderTop:"1px solid var(--c-border)", paddingTop:14 } : {}) }}>
                ...
                {examples.map((ex, i) => <Check key={i} on={checks[i]} label={ex} onChange={() => onToggle(age, i)} />)}
...
function Domain({ domain, data, onToggleCheck, onPresent }) {
  ...
  return (
    <div style={{ border:`1.5px solid ${borderColor}`, borderRadius:"var(--radius-sm)", padding:"12px 14px", marginBottom:10, background: data.present === true ? "var(--c-yes-soft)" : data.present === false ? "var(--c-no-soft)" : "var(--c-card)", transition:"all .2s" }}>
      ...
      {showEx && <div style={{ marginTop:8 }}>{domain.items.map((it, i) => <Check key={i} on={data.checks[i]} label={it} onChange={() => onToggleCheck(i)} />)}</div>}
    </div>
  );
}
...
function Badge({ value, max, label }) {
  ...
  return (
    <div style={{ textAlign:"center", flex:1 }}>
      <div style={{ width:56, height:56, borderRadius:"50%", margin:"0 auto 4px", background:`conic-gradient(${color} ${pct*360}deg, var(--c-border) 0deg)`, display:"flex", alignItems:"center", justifyContent:"center" }}>
...
export default function DivaApp() {
  ...
  useEffect(() => { const d = load(); if (d && d.inatt && d.hyper && d.impAdult && d.impChild) setS(d); setReady(true); }, []);
  ...
  const toggleEx = useCallback((cat, idx, age, exIdx) => {
    setS((p) => {
      ...
      const f = age === "adult" ? "ac" : "cc", pf = age === "adult" ? "ap" : "cp";
      const arr = [...p[cat][idx][f]]; arr[exIdx] = !arr[exIdx];
      const u = { ...p[cat][idx], [f]: arr };
      const cnt = arr.filter(Boolean).length;
      if (cnt > arr.length / 2) u[pf] = true;
      else if (cnt === 0 && u[pf] === true) u[pf] = null;
      next[cat][idx] = u; return next;
    });
  }, []);

  const setPresent = useCallback((cat, idx, field, val) => {
    setS((p) => { const n = { ...p, [cat]: [...p[cat]] }; n[cat][idx] = { ...p[cat][idx], [field]: val }; return n; });
  }, []);

  const toggleImpCheck = useCallback((ak, dk, idx) => {
    setS((p) => {
      const n = { ...p, [ak]: { ...p[ak] } }; const d = { ...p[ak][dk] };
  ...
  const setImpPresent = useCallback((ak, dk, val) => {
    setS((p) => { const n = { ...p, [ak]: { ...p[ak] } }; n[ak][dk] = { ...p[ak][dk], present: val }; return n; });
  }, []);
  ...
  const impAD = Object.values(s.impAdult).filter(d => d.present===true).length;
  const impCD = Object.values(s.impChild).filter(d => d.present===true).length;
  ...
  const aMet = aA>=6 && aC>=6, hMet = hA>=6 && hC>=6, impMet = impAD>=2 && impCD>=2;
  ...
  /* progress — dopamine micro-rewards */
  const answered = [...s.inatt,...s.hyper].filter(d => d.ap!==null && d.cp!==null).length;
  const domAnswered = [...Object.values(s.impAdult),...Object.values(s.impChild)].filter(d => d.present!==null).length;
  const totalSteps = 18 + 10 + 2; // symptoms + domains + onset/criterionE
  const doneSteps = answered + domAnswered + (onsetOk?1:0) + (s.criterionE!==null?1:0);
  const pct = Math.round(doneSteps/totalSteps*100);

  const handleReset = () => { if (confirm("Réinitialiser toutes les réponses ?")) { setS(init()); setTab(0); setSub(0); try { localStorage.removeItem(KEY); } catch {} } };
  ...
  const catDone = s[cat].filter(d => d.ap!==null && d.cp!==null).length;
  ...
  return (
    <div style={{ minHeight:"100vh", paddingBottom:100 }}>
      ...
      {/* ── HEADER ── */}
      <div style={{ background:`linear-gradient(135deg, var(--c-header-from) 0%, var(--c-header-to) 100%)`, padding:"28px 20px 20px", color:"#fff" }}>
        ...
        <div style={{ marginTop:16 }}>
          ...
          <div style={{ height:6, borderRadius:3, background:"rgba(255,255,255,0.15)", overflow:"hidden" }}>
            <div style={{ height:"100%", width:`${pct}%`, borderRadius:3, background:"#5ec4a8", transition:"width .4s ease" }} />
          </div>
        </div>
        ...
        <div style={{ display:"flex", gap:8, marginTop:14, flexWrap:"wrap" }}>
          {[{l:"A adulte",v:aA,ok:aA>=6},{l:"A enfance",v:aC,ok:aC>=6},{l:"H/I adulte",v:hA,ok:hA>=6},{l:"H/I enfance",v:hC,ok:hC>=6}].map(({l,v,ok})=>(
            <div key={l} style={{ background: ok ? "rgba(94,196,168,.25)" : "rgba(255,255,255,.1)", borderRadius:7, padding:"4px 10px", fontSize:11, fontWeight:600, display:"flex", alignItems:"center", gap:5 }}>
      ...
      <div style={{ maxWidth:620, margin:"0 auto", padding:"14px 12px" }}>
        ...
        {tab===0 && (<>
          ...
          <div style={{ marginBottom:12 }}>
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:"var(--c-text-soft)", marginBottom:4 }}>
              <span>{catDone}/{items.length} critères complétés</span>
              {catDone === items.length && <span style={{ color:"var(--c-yes)", fontWeight:700 }}>✓ Terminé</span>}
            </div>
            <div style={{ height:5, borderRadius:3, background:"var(--c-border)", overflow:"hidden" }}>
              <div style={{ height:"100%", width:`${Math.round(catDone/items.length*100)}%`, borderRadius:3, background:"var(--c-progress)", transition:"width .4s ease" }} />
            </div>
          </div>

          <div style={{ fontSize:12, color:"var(--c-text-soft)", margin:"0 0 14px", padding:"9px 12px", background:"var(--c-card)", borderRadius:"var(--radius-sm)", lineHeight:1.5, border:"1px solid var(--c-border)" }}>
        ...
        {tab===1 && (<>
          ...
          <div style={{ background:"var(--c-card)", borderRadius:"var(--radius)", padding:16, marginBottom:14, border:"1px solid var(--c-border)", boxShadow:"var(--shadow)" }}>
            <div style={{ fontFamily:"var(--font-display)", fontSize:15, fontWeight:700, marginBottom:10 }}>Critère B — Âge de début</div>
            ...
            {s.onsetBefore7===false && (
              <div style={{ marginTop:10, display:"flex", alignItems:"center", gap:8 }}>
                ...
                <input type="number" min="0" max="99" value={s.onsetAge} onChange={(e)=>setS(p=>({...p,onsetAge:e.target.value}))} style={{ width:56, padding:"5px 8px", borderRadius:8, border:"1px solid var(--c-border)", fontSize:14, fontWeight:600, textAlign:"center" }} />
          ...
          <div style={{ background:"var(--c-card)", borderRadius:"var(--radius)", padding:16, marginBottom:14, border:"1px solid var(--c-border)", boxShadow:"var(--shadow)" }}>
            ...
            <div style={{ fontSize:11, color: impAD>=2 ? "var(--c-yes)" : "var(--c-text-faint)", fontWeight:600, marginBottom:12 }}>{impAD}/5 domaines impactés (≥ 2 requis)</div>
            {Object.entries(IMPAIRMENT.adult).map(([k,dom])=><Domain key={k} domain={dom} data={s.impAdult[k]} onToggleCheck={(i)=>toggleImpCheck("impAdult",k,i)} onPresent={(v)=>setImpPresent("impAdult",k,v)} />)}
          </div>
          ...
          <div style={{ background:"var(--c-card)", borderRadius:"var(--radius)", padding:16, marginBottom:14, border:"1px solid var(--c-border)", boxShadow:"var(--shadow)" }}>
            ...
            <div style={{ fontSize:11, color: impCD>=2 ? "var(--c-yes)" : "var(--c-text-faint)", fontWeight:600, marginBottom:12 }}>{impCD}/5 domaines impactés (≥ 2 requis)</div>
            {Object.entries(IMPAIRMENT.child).map(([k,dom])=><Domain key={k} domain={dom} data={s.impChild[k]} onToggleCheck={(i)=>toggleImpCheck("impChild",k,i)} onPresent={(v)=>setImpPresent("impChild",k,v)} />)}
          </div>

          {/* Criterion E */}
          <div style={{ background:"var(--c-card)", borderRadius:"var(--radius)", padding:16, marginBottom:14, border:"1px solid var(--c-border)", boxShadow:"var(--shadow)" }}>
            <div style={{ fontFamily:"var(--font-display)", fontSize:15, fontWeight:700, marginBottom:10 }}>Critère E — Diagnostic différentiel</div>
            ...
            {s.criterionE===true && <input type="text" placeholder="Précisez le trouble…" value={s.criterionEDetail} onChange={(e)=>setS(p=>({...p,criterionEDetail:e.target.value}))} style={{ marginTop:10, width:"100%", padding:"9px 12px", borderRadius:"var(--radius-sm)", border:"1px solid var(--c-border)", fontSize:13 }} />}
          </div>
        </>)}
        ...
        {tab===2 && (<>
          <div style={{ background:"var(--c-card)", borderRadius:"var(--radius)", padding:20, marginBottom:14, border:"1px solid var(--c-border)", boxShadow:"var(--shadow)" }}>
            <div style={{ fontFamily:"var(--font-display)", fontSize:17, fontWeight:800, marginBottom:18, textAlign:"center" }}>Résumé des symptômes</div>
          ...
          <div style={{ background:"var(--c-card)", borderRadius:"var(--radius)", padding:18, marginBottom:14, border:"1px solid var(--c-border)", boxShadow:"var(--shadow)" }}>
            <div style={{ fontFamily:"var(--font-display)", fontSize:17, fontWeight:800, marginBottom:14, textAlign:"center" }}>Formulaire de cotation</div>
            {[
              ...
              {l:"Critères C/D : Retentissement ≥ 2 domaines (adulte)",ok:impAD>=2},
              {l:"Critères C/D : Retentissement ≥ 2 domaines (enfance)",ok:impCD>=2},
          ...
          {/* Diagnosis */}
          <div style={{ background: dx ? `linear-gradient(135deg, var(--c-header-from), var(--c-header-to))` : "var(--c-card)", borderRadius:"var(--radius)", padding:22, marginBottom:14, border: dx?"none":"1px solid var(--c-border)", boxShadow:"var(--shadow-lg)", textAlign:"center" }}>
            ...
            {dx ? (<>
              ...
              <div style={{ fontSize:15, fontWeight:700, marginBottom:6 }}>
                {(aMet||hMet) && !impMet && !(impADone&&impCDone) ? "Complétez le retentissement pour obtenir le résultat"
                  : (aMet||hMet) && !impMet ? "Diagnostic TDAH non retenu"
              ...
              <div style={{ fontSize:12, color:"var(--c-text-soft)", lineHeight:1.5 }}>
                {!aMet && !hMet ? "Le seuil de 6 symptômes n'est atteint dans aucun domaine pour les deux âges de vie."
                  ...
                  : !impMet ? "Les critères symptomatiques sont atteints, mais le retentissement n'est pas présent dans au moins 2 domaines de vie aux deux âges."
          ...
          <div style={{ padding:"12px 14px", background:"#eef6f3", borderRadius:"var(--radius-sm)", fontSize:11, color:"var(--c-accent-dark)", lineHeight:1.5, marginBottom:14, border:"1px solid var(--c-border)" }}>
            ⚠️ Cet outil est une aide à la passation de la DIVA 2.0 et ne remplace pas le jugement clinique d'un professionnel de santé qualifié. Le diagnostic de TDAH doit être posé par un clinicien formé, en prenant en compte l'ensemble du tableau clinique.
          </div>
